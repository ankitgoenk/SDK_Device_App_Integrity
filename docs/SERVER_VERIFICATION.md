...
| 10 | A key id names the key it was derived from | `InMemoryEnrolledKeys.enrol` derives it via `KeyIds` rather than taking it as a parameter, and `ReportVerifierTest`'s rig asserts the derived id equals the signing side's; one mutant. `keyIdOf`'s KDoc claimed "the backend recomputes it from the key it has enrolled and compares" while the derivation sat in an Android library a JVM backend cannot consume |
| 11 | An enrolled key belongs to the submitter presenting it | **Not enforced.** `EnrolledKeys.find` is a global lookup by the claimed id, `ReportVerifier.check` never reads `header.packageName` — the field carried across the signature boundary for precisely this — and it never sees `sessionId`. Item 10 removed the *chosen* id; nothing yet binds a key to an owner, so do not describe that as verified |
