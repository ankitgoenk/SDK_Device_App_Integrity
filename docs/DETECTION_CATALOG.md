...
| `ROOT_PROP_SPOOF` | **Planned, and the technique as written is not implementable from an app.** Both named ground-truth sources are privileged: `/system/build.prop` is mode `0600 root`, and `/proc/bootconfig` is denied even to shell. `/proc/cmdline` *is* app-readable but carries no vbmeta entries on Android 12+, where boot parameters moved to bootconfig. Measured on a Pixel 10a (TESTING.md §9), where the spoofing is real and invisible: bootconfig says `vbmeta.device_state="unlocked"` while the property reports `locked`. A redesign must compare **app-visible values against each other**, not against privileged truth | NAT | H | low | Redesign pending; the id stays because ids are deprecated, never reused |
