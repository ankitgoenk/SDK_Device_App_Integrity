...
| `ROOT_DANGEROUS_PROPS` | **Partly implemented (JVM, phase 2). Detects a modified *build*, not root** — the two stopped coinciding when root moved into the kernel. Measured blind on a KernelSU Next Pixel 10a whose system image is genuinely `user`/`release-keys` (TESTING.md §9): nothing is spoofed, the check is simply answering a different question than the one its name suggests. Do not treat silence here as evidence about root. `Build.TAGS` contains `test-keys`, `Build.TYPE` in {`userdebug`,`eng`}. The `ro.debuggable` / `ro.secure` / `service.adb.root` reads were planned for the native property API in phase 3. **Measured 2026-09-02 on a Pixel 10a (API 36): `ro.debuggable` and `ro.secure` return an *empty string* to an app**, the same SELinux-labelled-property class as `ro.bootimage.build.fingerprint`. They read fine from `adb shell` and fine from an app on Android 11, so this is a platform-version restriction rather than a per-device one. Native does not help: the restriction is on the property label, not the calling language. Treat empty as unreadable, never as a value — and do not plan phase-3 work on these three without re-measuring the target API level first | JVM+NAT | L | **high** | `resetprop` rewrites all of these in seconds and hiding modules do so by default. Only the native comparison against on-disk values (`ROOT_PROP_SPOOF`) sees through it |
| `ROOT_PROP_SPOOF` | **Implemented (JVM, phase 2).** Compare each `android.os.Build` field against the system property that backs it — `Build.FINGERPRINT` vs `ro.build.fingerprint`, and seven more. Both surfaces are app-visible and need no privilege. **Not a partition comparison:** the obvious design, checking `ro.system.build.fingerprint` against `ro.build.fingerprint`, was measured producing **26 disagreements on a stock unrooted Redmi against 3 on the rooted Pixel** — under Treble the system image can come from the SoC vendor and vendor/odm from the OEM, so partitions differing is normal. `Build.FINGERPRINT` being *initialised from* `ro.build.fingerprint` is an invariant the framework maintains, so divergence is a broken invariant rather than a heuristic. Unreadable properties are excluded, never counted as difference | JVM | H | **none observed** | Rewrite the property too, or hook `SystemProperties.get`. Nothing prevents either — it is only more work, which is why the spoofers in the wild skip it: Play Integrity Fork rewrites `Build` for `com.google.android.gms.unstable` and leaves the property alone. Catches a prevalent lazy technique, not a determined attacker |
...
| `HOOK_XPOSED_STACK` | **Measured blind (2026-09-01, TESTING.md §9).** Throw and inspect the stack trace for `de.robv.android.xposed.*`, `LSPosed`, `EdXposed` frames. Under a live hook: **90 frames, 0 suspicious, byte-identical to clean.** LSPlant-style hooking rewrites the ART method entry point and contributes no frame of its own | JVM | H | low | The technique assumes a Java-level bridge frame that modern frameworks do not produce |
| `HOOK_XPOSED_CLASSES` | **Measured blind (2026-09-01, TESTING.md §9). Do not build without re-measuring.** `Class.forName("de.robv.android.xposed.XposedBridge")` and helpers on all classloaders. Under a *live* hook from Vector 2.2, neither `XposedBridge`, `XposedHelpers` nor `io.github.libxposed.api.XposedInterface` resolves — the reading is identical to a clean device. A modern framework injects its API into the **module's** classloader, not the hooked app's | JVM | M | low | Nothing to hide: the class was never in the target app's classloader to begin with |
...
| `HOOK_SELF_TEXT_MISMATCH` | **Implemented (NAT, phase 3a).** Compare this SDK's own `r-xp` mapping against the `.so` file it was loaded from; report evidence consistent with post-load code modification. The module is located by the address of one of its own functions, never by a path substring — a `.so` stored uncompressed is mapped from `…/base.apk` and the library's name appears nowhere in the mapping table | NAT | H | low | Hook the reading path (`pread`/`open`), restore on read, or redirect through GOT/PLT and never touch `.text` at all — its absence proves nothing. Design: `docs/detectors/HOOK_SELF_TEXT_MISMATCH.md` |
...
| `HOOK_UNEXPECTED_MODULE` | **Implemented (JVM, phase 3) — the hook family's first detector with a positive control (2026-09-01, TESTING.md §9).** **Executable, genuinely file-backed** mappings in `/proc/self/maps` outside `/system`, `/apex`, `/vendor`, `/product`, `/system_ext`, `/data/app`, `/data/misc/apexdata/com.android.art/`. **"File-backed" is not "starts with a slash":** the kernel's anonymous-memory facilities give regions path-shaped names and ART allocates its JIT cache from them, so every process running managed code carries an executable region matching no allow-listable prefix. **Which facility depends on the platform version:** `/memfd:jit-cache (deleted)` on Android 13 and 16, `/dev/ashmem/jit-zygote-cache_<pid>_<tid> (deleted)` on Android 11. Both namespaces are excluded by prefix; a trailing ` (deleted)` on a real path is not, because a library unlinked after loading is worth reporting. The two qualifiers carry the detector: matching *all* mappings leaves 113 unexplained paths, and requiring executable+file-backed drops that to **0 clean / 1 hooked / 0 on the second device**. Evidence is the mapped path — note the app can read this out of its own maps while being denied `stat` on the very file it is executing | JVM | M | **low, measured** | Nothing forces a framework to stay resident; ReZygisk unloads from the forked child and is invisible to this. Catches residents, not visitors — and a framework free to relocate under `/data/app` would defeat the prefix list |
| `HOOK_MAPS_INCONSISTENT` | **Measured blind on two hooking stacks; `DOCUMENT` in [DETECTION_TRIAGE.md](DETECTION_TRIAGE.md) §3.** Proposed as an outside view of the process compared against the app's own `/proc/self/maps`, reporting any mapping present in one and absent from the other. **No divergence exists on either stack measured (TESTING.md §9):** on `K1` root's view and the app's own view are byte-identical, and ReZygisk *unloads* from the forked child rather than hiding — 6 zygisk mappings in `zygote64`, 0 in the app, in both views. Re-tested on `K2`, where Vector stays **resident** in the hooked process and still does not hide: 3 mappings, both views. A framework that never unloads is the best case this check could have, and there is still nothing to observe, so there is no positive control to construct | NAT | H | low | Nothing to bypass: on every framework measured the technique has no signal to detect. **Compare address ranges, never path strings** — a `.so` loaded from an APK maps under `…/base.apk`, so a name comparison fires on every device, and `vector` as a substring matches ART's own `dalvik-Concurrent mark-compact chunk-info vector` on every Android device ever shipped. Revisit only if a framework appears that scrubs its own maps entry rather than cleaning up after itself; `HOOK_UNEXPECTED_MODULE` covers the resident case by a route that works |
...
| `APP_DEX_DIGEST_MISMATCH` | **Implemented (JVM, phase 4).** Aggregate SHA-256 over every `classes*.dex` entry of the running APK, compared against the value `integrity-baseline-plugin` computed for the build. The aggregate construction lives in `integrity-model` so the build and the device cannot compute it differently. **Declines on split installs** — the baseline covers the base APK, so comparing part of an app against a whole one would make Play Feature Delivery look like tampering, and this id escalates decisively. With no baseline configured the measurement is still reported, for a backend holding the baseline to compare. Escalates decisively only once weighted above `INFORMATIONAL` via `AppDetectors.proposedWeights` — `RiskScorer` filters unpromoted signals out of every escalation rule | JVM | H | med | Everything that defeats a measurement taken by the code being measured: a hooked `ZipFile`/`open` returns the original bytes, a patched detector returns a matching digest. On a rooted device a match means only "the client says it matches", which is why the measurement travels in the report rather than merely being acted on |
...
| `ENV_NO_DEVICE_LOCK` | **Implemented (JVM, phase 5).** `KeyguardManager.isDeviceSecure()` — true when a PIN, pattern or password is set. API 23, no permission, and `minSdk` is 24, so it needs no version gate. **Fires on absence**, never on presence: a secured device produces no signal, because a signal that appeared when the device was locked would be a report raising trust, which hard rule 9 forbids outright. Unreadable (no `KeyguardManager`) is `INCONCLUSIVE` | JVM | L | **high** | Bypass: one framework call, so hooking `isDeviceSecure` to return true is a one-liner — the same ceiling every JVM check in this catalogue has. **There is deliberately no separate biometric signal.** Android will not enrol a face or fingerprint without a PIN/pattern/password backup, so `isDeviceSecure()` is already true whenever one exists; a `ENV_NO_BIOMETRIC_ENROLLED` would add nothing to "is this device locked" and would fire on every secure PIN-only device. Nor can face be told from fingerprint: `BiometricManager` reports STRONG vs WEAK class, not modality, and `FEATURE_FACE` reports hardware rather than enrolment. FP: **high, and made of real people** — shared family tablets, kiosks, users who decline a lock screen. Combine, never enforce alone |
...
| `SRV_REPORT_LABEL_UNRECOGNISED` | **Implemented (SRV, phase 7).** A submission carried a `category` or `confidence` string this build does not recognise. Raised alongside the evidence that arrived, never instead of it | SRV | I | **high** | No bypass: it fires *because* a label did not parse, and the parse is the backend's. The reason it exists is that the previous behaviour was silent — an unrecognised `category` fell back to `META` and an unrecognised `confidence` to `INCONCLUSIVE`, the two values carrying no weight, so one altered character removed a `CONFIRMED` hooking signal from the escalation rule keyed on its category. FP risk is high and structural: **version skew is the likely cause**, not an attack. A fleet running an SDK whose `Category` this backend predates raises it on every report, which is the point — that case used to be invisible. Category is now derived from the id via `SignalCategories`, so this fires on `confidence` and on ids in no known family |
...
| `META_VISIBILITY_RESTRICTED` | **Implemented (JVM, phase 2).** Package visibility denied, so package-based results are incomplete. Emitted alongside the affected check's own `INCONCLUSIVE`, because the two answer different questions: that check could not conclude, versus this report was assembled without package visibility at all. Expect it on **every** API 30+ device today — `RealPackageProbe.absenceIsConclusive` is `SDK_INT < R || hasQueryAllPackages()`, and ADR-0004 forbids `QUERY_ALL_PACKAGES`, so the branch is permanently false. Now that `<queries>` declares the managers explicitly the platform does guarantee their visibility, so absence of a *declared* package is conclusive and this should narrow; revisiting that is a change to what absence means, not a defect fix |
