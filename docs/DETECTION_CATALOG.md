...
| `ROOT_PROP_SPOOF` | **Planned, and the technique as written is not implementable from an app.** Both named ground-truth sources are privileged: `/system/build.prop` is mode `0600 root`, and `/proc/bootconfig` is denied even to shell. `/proc/cmdline` *is* app-readable but carries no vbmeta entries on Android 12+, where boot parameters moved to bootconfig. Measured on a Pixel 10a (TESTING.md §9), where the spoofing is real and invisible: bootconfig says `vbmeta.device_state="unlocked"` while the property reports `locked`. A redesign comparing **app-visible values against each other** was built and measured on `K3` (K1 + Play Integrity Fork v18) on 2026-09-01, and **both halves failed**. There is nothing to find: PIF writes no `ro.*.build.fingerprint` at all, and its `android.os.Build` spoofing is scoped to `com.google.android.gms.unstable` and `com.android.vending` — attestation spoofers target the **attestation client, not the device**, so a third-party process observes nothing. And the naive form is a false-positive generator: `ro.bootimage.build.fingerprint` is **app-unreadable and returns empty** while `adb shell` reads it fine, so "all partitions agree" reports a mismatch on a clean device. Treat empty as unreadable-per-property, never as difference | NAT | H | low | Not "redesign pending" any more — the redesign was measured blind. The id stays because ids are deprecated, never reused |
