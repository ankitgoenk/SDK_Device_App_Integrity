...
Nine device detections are live against a catalogue of 70 — the other ten `SignalId`s in
code are `META_*` (the SDK reporting on itself, seven of them), `ATT_*` (server-side vocabulary
we never emit) and two `SRV_*` the backend does emit while verifying a submission, so "19
implemented" from the catalogue gate is not nineteen detections. Run
